...
def тренд(ряд_отн):
    """Куда тема поехала: сравниваем первый месяц с предпоследним.

    Последний месяц обрезан границей выгрузки — по нему судить нельзя.
    """
    if len(ряд_отн) < 3 or ряд_отн[0] < 0.3:
        return ""
    было, стало = ряд_отн[0], ряд_отн[-2]
